...
select_kernel_version() {
    ...
    echo "----------------------------------------"
    echo -e "Текущая дата:      \033[1;36m2025-02-15 04:32:51 UTC\033[0m"
    ...
    echo "1) Стабильная      - Оптимальный баланс производительности и стабильности"
    echo "2) Edge            - Новейшие функции и обновления, может быть менее стабильной"
    echo "3) RT              - Оптимизирована для задач реального времени"
    echo "4) LTS             - Долгосрочная поддержка, максимальная стабильность"
    
    # Дополнительная информация
    echo -e "\n\033[1;33m🔧 Особенности версий:\033[0m"
    echo "• Стабильная - рекомендуется для большинства пользователей"
    echo "• Edge       - подходит для энтузиастов и тестировщиков"
    echo "• RT         - для аудио/видео обработки в реальном времени"
    echo "• LTS        - для серверов и рабочих станций"
    
    echo -e "\n\033[1;32mℹ️  Все версии будут установлены с оптимизацией ${PSABI_VERSION}\033[0m"
    echo "----------------------------------------"
    
    # Запрос выбора версии
    echo ""
    read -rp $'\033[1;33mВыберите версию ядра (1-4, по умолчанию 1): \033[0m' choice
